...
struct node {
    Data d;
    struct node* rest;   // pekar på nästa nod (resten av listan)
};
...
public:
    ...
    // Tillåt flytt (move). Det gör att List kan returneras från funktioner (map).
    // Vi förbjuder kopiering (ovan) för att undvika dubbel-free,
    // men vi tillåter flytt så att man kan returnera List från funktioner.”
    ...
    // Skriver ut listan till given stream.
    ...
    /*
      ...
      lokala variabler utanför funktionen. Därför skickar vi med en extra pekare
      (void* ctx) som callbacken kan använda för att nå extra data.
    ...
    */
    ...
    List map(Data(*f)(Data)) const;

    Data fold(Data init, Data(*f)(Data acc, Data x)) const;
};
