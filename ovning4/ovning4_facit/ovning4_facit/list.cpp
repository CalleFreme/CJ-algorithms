...
static Data nodeFold(node* l, Data init, Data(*f)(Data, Data)) {
    Data acc = init;
    node* cur = l;
    while (cur) {
        acc = f(acc, cur->d);
...
// Move-konstruktor
List::List(List&& other) noexcept : start(other.start) {
    other.start = nullptr;
}
...
// Map-implementation
// 
// Kontext för map: vi behöver veta
// - var vi bygger resultatlistan
// - vilken funktion f vi ska applicera
struct MapCtx {
    List* out;
    Data(*f)(Data);
};

static void map_cb(Data x, void* ctx) {
    MapCtx* c = static_cast<MapCtx*>(ctx);
    Data y = c->f(x);
    // Lägg sist så att ordningen bevaras
    (void)c->out->addLast(y);
}

List List::map(Data(*f)(Data)) const {
    List r;
    MapCtx ctx{ &r, f };
    forEachCtx(map_cb, &ctx);
    return r;
}
...
// 
// Kontext för fold:
// - ackumulatorn (acc) som vi uppdaterar
// - f som berättar hur vi kombinerar acc och x
struct FoldCtx {
    Data acc;
    Data(*f)(Data acc, Data x);
};

static void fold_cb(Data x, void* ctx) {
    FoldCtx* c = static_cast<FoldCtx*>(ctx);
    c->acc = c->f(c->acc, x);
}

Data List::fold(Data init, Data(*f)(Data acc, Data x)) const {
    FoldCtx ctx{ init, f };
    forEachCtx(fold_cb, &ctx);
    return ctx.acc;
}