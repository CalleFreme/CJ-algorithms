...
int main() {
    std::puts("ADT-övning: snäll testkörning (PASS/FAIL)\n");
    ...
    check(l.isEmpty(), "List: ny lista ska vara tom");

    check(l.addFirst(2) == 1, "List: addFirst(2) ska lyckas");
    check(l.addFirst(1) == 1, "List: addFirst(1) ska lyckas");
    check(l.addLast(3) == 1, "List: addLast(3) ska lyckas");

    std::fputs("List l = ", stdout);
    l.printList(stdout);

    check(l.lengthOfList() == 3, "List: lengthOfList() ska vara 3");
    check(!l.isEmpty(), "List: lista ska inte vara tom efter inserts");
    check(l.isInList(2) == 1, "List: isInList(2) ska vara true");
    check(l.isInList(99) == 0, "List: isInList(99) ska vara false");

    // getFirstElement / getLastElement har precondition: listan får inte vara tom
    if (!l.isEmpty()) {
        check(l.getFirstElement() == 1, "List: getFirstElement() ska vara 1");
        check(l.getLastElement() == 3, "List: getLastElement() ska vara 3");
    }
    else {
        check(false, "List: kan inte testa getFirst/getLast på tom lista");
    }

    std::fputs("List forEach: ", stdout);
    l.forEach(print_elem);
    std::puts("");
    ...
    // Om Stack::empty/size är TODO och assertar, kommer programmet stoppa här.
    // När de är implementerade, kör dessa tester:
    check(st.empty(), "Stack: ny stack ska vara tom");
    check(st.size() == 0, "Stack: size() ska vara 0 från start");
    ...
    if (!st.empty()) {
        ...
        check(false, "Stack: kan inte testa peek/pop på tom stack");
    }

    st.clear();
    check(st.empty(), "Stack: clear() ska göra stacken tom");
    ...
    std::fputs("Set a efter remove(2) = ", stdout);
    a.print(stdout);
