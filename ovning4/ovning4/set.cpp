...
// forEachCtx anropar en callback för varje element och skickar även med
// en kontextpekare (Ctx = Context) som callbacken kan använda.
// 
// Vad är en callback? En funktion som skickas som argument till en annan funktion.
// Vad är en kontext? Extra data som callbacken kan använda för att göra sitt jobb.
//
// Nedan finns en tydlig struktur med kontext-structs + callbacks.
// Du behöver fylla i TODO-delarna i union/snitt/diff.

// 1) UNION: lägg in allt från a och b i resultatet (add tar hand om dubbletter)
namespace {
    struct UnionCtx {
        Set* out;
    };
...
Set Set::set_union(const Set& a, const Set& b) {
    // TODO:
    // - Skapa Set r
    // - Iterera a och b med forEachCtx och lägg in i r
    //
    // Hint:
    // Set r;
    // UnionCtx ctx{&r};
